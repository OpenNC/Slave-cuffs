...
string g_szLockCmd="bLock"; // message for setting lock on or off
...
string g_szHideCmd="Blindfold"; // Comand for Blindfold to hide
...
{// message for cuff received;
    ...
    { // a change of colors has occured, make sure the cuff try to set identiccal to the collar
        ...
        if (g_nHidden)
        {
            ...
            llOwnerSay("@setenv_daytime:0.5=force,showhovertextall=y,shownames=y");
        }
        else
        {
            SetAllElementsAlpha (1.0);
            if (blackout == "on")
                llOwnerSay("@setenv_bluedensityr:0.0=force,@etenv_bluedensityb:0.0=force,setenv_bluedensityg:0.0=force,setenv_hazedensity:0.0=force,showhovertextall=n,shownames=n");
        }
    }
    else  if (nStartsWith(szMsg,"blackout"))
    {
        list lstCmdList = llParseString2List( szMsg, [ "=" ], [] );
        blackout= llList2String(lstCmdList,1);
        if ((blackout == "on") && (!g_nHidden))
            llOwnerSay("@setenv_bluedensityr:0.0=force,@etenv_bluedensityb:0.0=force,setenv_bluedensityg:0.0=force,setenv_hazedensity:0.0=force,showhovertextall=n,shownames=n");
        else
            llOwnerSay("@setenv_daytime:0.5=force,showhovertextall=y,shownames=y");
    }
